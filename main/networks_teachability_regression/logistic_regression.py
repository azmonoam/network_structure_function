...
class LogisticRegression(torch.nn.Module):
    def __init__(
        ...
        input_dim, output_dim = layers_size
...
def logistic_regression_learning(
        layers_size: List[int],
    ...
    out_path = f"{output_path}/{task}_{time_str}_lr_{learning_rate}_bs_{batch_size}_lm_{lambda_reg}_output{sufix}"

    if layers_size[0] != train_data[0][0].shape[0]:
        layers_size[0] = train_data[0][0].shape[0]
    ...
    for epoch in range(epochs):
        ...
        for train_input, train_labels in train_loader:
            ...
            all_linear_params = torch.cat([x.view(-1) for x in model.linear.parameters()])
            l1_regularization = lambda_reg * torch.norm(all_linear_params, 1)
            loss = criterion(predictions, train_labels.reshape(-1, 1)) + l1_regularization
        ...
        if epoch % test_every == 0:
            ...
            if all_epochs_mae_test[-1] < best_test_mse:
                best_model = model
    ...
    if save_preds:
        ...
        res['test_pred'] = test_pred.cpu()
        res['test_label'] = test_label_no_increse.cpu()
