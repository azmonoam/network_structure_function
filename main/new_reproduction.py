...
class NReproduction:
    ...
    def __init__(
        ...
        self.bias_mutation_probability = mutation_probabilities.get('bias_mutation_probability')
        self.connection_switch_mutation_probability = mutation_probabilities.get(
            'connection_switch_mutation_probability')
        self.connection_addition_mutation_probability = mutation_probabilities.get(
            'connection_addition_mutation_probability')
        self.connection_removal_mutation_probability = mutation_probabilities.get(
            'connection_removal_mutation_probability')
        self.weight_mutation_probability_factor = mutation_probabilities.get(
            'weight_mutation_probability_factor')
        self.neuron_addition_mutation_probability = mutation_probabilities.get(
            'neuron_addition_mutation_probability')
        self.neuron_removal_mutation_probability = mutation_probabilities.get(
            'neuron_removal_mutation_probability')
        self._init_org_properties()
        self.layer_neuron_idx_mapping = [[] for _ in range(self.num_layers + 1)]
        self.edges = []

    def mate(self) -> Organism:
        self._check_values()
        neurons_from_parents = self._get_neurons_and_assign_biases_from_parents()
        ...
        return Organism(
            ...
            allowed_weights_values=self.allowed_weights_values,
            allowed_bias_values=self.allowed_bias_values,
            communities_input_symmetry=self.communities_input_symmetry,
    ...
    def _add_parents_edges(
        ...
        while len(self.edges) != self.max_allowed_connections:
            ...
            elif len(self.edges) > self.max_allowed_connections:
                self._remove_connection()
    ...
    def _add_extra_connection(
        ...
        if len(random_parent_unused_nodes_edges) < 1:
    ...
    def _init_org_properties(self):
        self.num_layers = self.first_parent.num_layers
        ...
        self.communities_input_symmetry = self.first_parent.communities_input_symmetry
        self.bias_list = [[] for _ in range(len(self.first_parent.biases))]
        self.max_allowed_connections = random.choice(
            [
                self.first_parent.max_allowed_connections,
                self.second_parent.max_allowed_connections,
            ]
        )
