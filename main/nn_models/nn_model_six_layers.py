...
class FeedforwardNeuralNetModelSixLayers(NeuralNetModel):
    def __init__(
            self,
        ...
        super(FeedforwardNeuralNetModelSixLayers, self).__init__(weight_mask, bias_mask, dimensions, reinitiate,
                                                                 activate)
        ...
        self.liner_hidden_to_hidden_c = prune.custom_from_mask(
            nn.Linear(dimensions[3], dimensions[4]),
            name='weight',
            mask=weight_mask[3].T,
        )
        self.liner_hidden_to_hidden_c = prune.custom_from_mask(
            self.liner_hidden_to_hidden_c,
            name='bias',
            mask=bias_mask[3],
        )
        self.liner_hidden_to_hidden_d = prune.custom_from_mask(
            nn.Linear(dimensions[4], dimensions[5]),
            name='weight',
            mask=weight_mask[4].T,
        )
        self.liner_hidden_to_hidden_d = prune.custom_from_mask(
            self.liner_hidden_to_hidden_d,
            name='bias',
            mask=bias_mask[4],
        )
    ...
    def forward(
        ...
        out = self.activate(out)
        out = self.liner_hidden_to_hidden_c(out)
        out = self.activate(out)
        out = self.liner_hidden_to_hidden_d(out)
        out = self.activate(out)
    ...
    def _update_initial_params(self):
        ...
        self.liner_hidden_to_hidden_c.weight.data = torch.nn.init.xavier_uniform_(
            self.liner_hidden_to_hidden_c.weight.data)
        self.liner_hidden_to_hidden_c.bias.data.uniform_(-2.0, 2.0)
